...
Each position belongs to one or both categories:

- **FOH** = Customer-facing roles
- **BOH** = Kitchen/production roles

This categorization:
- Filters which employees appear for each position in ratings
...
Responsible for all cooked proteins. During service:
- Cook chicken filets and nuggets to spec
- Maintain proper holding times
- Keep station organized and stocked
- Communicate timing with expeditor

Excellence means consistent cook quality, 
fast recovery during rushes, and zero food safety issues.
```
...
The order affects how positions appear in dropdowns and lists.
...
2. Enter the Spanish name and description
...
<AccordionGroup>
  <Accordion title="Position doesn't appear in the rating form">
    ...
    - The position is active (not deleted)
  </Accordion>
