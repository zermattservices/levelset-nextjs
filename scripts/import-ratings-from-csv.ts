...
async function main() {
  ...
  // Create a Set of existing rating keys for quick lookup
  const existingKeys = new Set<string>();
  existingRatings.forEach(r => {
    const key = `${r.employee_id}|${r.rater_user_id}|${r.position}|${r.created_at}`;
  ...
  for (const row of rows) {
    ...
    const key = `${employeeId}|${raterId}|${row.position}|${createdAt}`;
