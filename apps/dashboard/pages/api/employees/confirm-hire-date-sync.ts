...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { notification_id, unmatched_mappings, employee_edits, kept_employees, termination_reasons } = req.body;
    ...
    if (terminatedEmployees.length > 0) {
      const terminationReasonsMap: Record<string, string> = termination_reasons || {};
      const today = new Date().toISOString().split('T')[0];

      for (const emp of terminatedEmployees) {
        const reason = terminationReasonsMap[emp.id] || null;
