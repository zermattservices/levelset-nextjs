  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (intent === 'create') {
        ...
        // full_name is a generated column in the database, so we don't include it in the insert
        // It will be automatically computed from first_name and last_name
        const employeeData: Partial<Employee> = {
          first_name,
          last_name,
          // full_name is generated by database, don't include it
          role,
          availability,
          is_foh: is_foh || false,
          is_boh: is_boh || false,
        ...
        if (shouldCalculatePay(location_id)) {
          ...
          // Use computed full_name for the calculation
      ...
      if (intent === 'update') {
        const { id, role, is_certified, certified_status, is_foh, is_boh, availability, active, termination_date, termination_reason, last_points_total } = req.body;
      ...
      if (last_points_total !== undefined) {
        updateData.last_points_total = last_points_total;
      }
