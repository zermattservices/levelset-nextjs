...
export function DocumentsPageContent({ config }: { config: DocumentsConfig }) {
  ...
  const [folderPath, setFolderPath] = React.useState<{ id: string | null; name: string }[]>([
    { id: null, name: config.rootBreadcrumbName },
  ]);
  ...
  const fetchFolders = React.useCallback(async () => {
    try {
      ...
      const res = await fetch(`${config.apiBasePath}/folders?${params}`, { headers });
      if (res.ok) {
  ...
  }, [authHeaders, currentFolderId, config.apiBasePath]);
  ...
  const fetchDocuments = React.useCallback(async () => {
    try {
      ...
      const res = await fetch(`${config.apiBasePath}?${params}`, { headers });
      if (res.ok) {
        const data = await res.json();
        setDocuments(data);
      }
  ...
  }, [authHeaders, currentFolderId, categoryFilter, searchQuery, config.apiBasePath]);
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      const headers = await authHeaders();
      const res = await fetch(`${config.apiBasePath}/folders`, {
        method: 'POST',
  ...
  const handleSaveDocumentField = async (field: string) => {
    ...
    try {
      ...
      const res = await fetch(`${config.apiBasePath}/${selectedDocument.id}`, {
        method: 'PUT',
        headers: { ...headers, 'Content-Type': 'application/json' },
  ...
  const handleDownload = async () => {
    ...
    try {
      const headers = await authHeaders();
      const res = await fetch(`${config.apiBasePath}/${selectedDocument.id}`, { headers });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 180,
      renderCell: (params) => {
        const color = config.categoryColors[params.value as string] || 'var(--ls-color-neutral)';
        return (
          <Chip
            label={config.categoryLabels[params.value as string] || params.value}
            size="small"
            sx={{
              fontFamily,
              fontSize: 11,
  ...
  return (
    <>
      <Head>
        <title key="title">{config.headTitle}</title>
        <meta key="og:title" property="og:title" content={config.headTitle} />
      </Head>

      {/* Back link */}
      {config.backLink && (
        <Link href={config.backLink.href} className={sty.backLink}>
      ...
      <div className={sty.pageHeader}>
        <h1 className={sty.pageTitle}>{config.pageTitle}</h1>
        {config.headerExtra}
      </div>
        ...
        <div className={sty.actionBarLeft}>
          ...
          <div className={sty.filterChips}>
            ...
            {config.categories.map((cat) => {
              const color = config.categoryColors[cat] || 'var(--ls-color-neutral)';
              return (
                <Chip
                  key={cat}
          ...
          <div className={sty.drawerContent}>
            ...
            <div className={sty.drawerSection}>
              ...
              {editingField === 'category' ? (
                ...
                >
                  <Chip
                    label={config.categoryLabels[selectedDocument.category] || selectedDocument.category}
                    size="small"
                    sx={{
                      fontFamily,
                      fontSize: 11,
                      fontWeight: 600,
                      backgroundColor: (config.categoryColors[selectedDocument.category] || 'var(--ls-color-neutral)') + '1a',
                      color: config.categoryColors[selectedDocument.category] || 'var(--ls-color-neutral)',
                      border: 'none',
                      height: 24,
                    }}
                  />
