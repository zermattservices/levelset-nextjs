...
export function LocationProvider({ children }: LocationProviderProps) {
  ...
  const fetchLocations = useCallback(async () => {
    ...
    try {
      ...
      console.log("[Location] Fetching for user:", {
        id: appUser.id,
        role: appUser.role,
      ...
      // Levelset Admins see ALL locations across all orgs.
      // Regular users are scoped to their org.
      if (fetchedLocations.length === 0) {
        const isAdmin = appUser.role === "Levelset Admin";
        console.log("[Location] Fallback query — isAdmin:", isAdmin, "org_id:", appUser.org_id);
        ...
        if (appUser.org_id && !isAdmin) {
          fallbackQuery = fallbackQuery.eq("org_id", appUser.org_id);
        }

        const { data, error: locError } = await fallbackQuery;
        if (locError) throw locError;
        fetchedLocations = data ?? [];
        console.log("[Location] Fallback returned", fetchedLocations.length, "locations");
      }
