...
export async function getEmployeeRatings(
  ...
  const cacheKey = `ratings:${employeeId}:${locationId ?? 'org'}:${limit}`;

  return tenantCache.getOrFetch(orgId, cacheKey, CacheTTL.DYNAMIC, () =>
    _getEmployeeRatings(orgId, employeeId, limit, locationId)
  );
}

/** Internal: uncached ratings query */
async function _getEmployeeRatings(
  orgId: string,
  employeeId: string,
  limit: number,
  locationId?: string
